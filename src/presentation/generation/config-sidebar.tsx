...
export function ConfigSidebar(props: ConfigSidebarProps): React.ReactElement {
  return (
    <aside className="app-surface grid h-fit gap-4 rounded-lg p-4 slide-up" style={{ animationDelay: "0.2s" }}>
      ...
      <ConfigRow label="Provider" value={props.selectedProvider?.name || "None"} />
      <ConfigRow label="Model" value={props.selectedProvider?.model || "None"} />
      <ConfigRow label="Temperature" value={String(props.selectedPreset?.temperature ?? props.selectedProvider?.defaultTemperature ?? "-")} />
      <ConfigRow label="Max Tokens" value={String(props.selectedPreset?.maxTokens ?? props.selectedProvider?.defaultMaxTokens ?? "-")} />
      <ConfigRow label="Prompt Template" value={props.selectedTemplate?.name || "None"} />
      <ConfigRow label="Output Format" value={props.selectedPreset?.outputFormat || "markdown"} />
      ...
      <div className="rounded-md bg-secondary p-3 text-sm text-secondary-foreground">
        {props.metadata.inputTokens !== undefined && props.metadata.outputTokens !== undefined ? (
          <>
            <div>Input: {props.metadata.inputTokens.toLocaleString()} tok</div>
            <div>Output: {props.metadata.outputTokens.toLocaleString()} tok</div>
            <div className="border-t pt-1 mt-1 font-medium">
              {(props.metadata.inputTokens + props.metadata.outputTokens).toLocaleString()} total
            </div>
