...
function parseJudgeReply(raw: string) {
  ...
  try {
    ...
    throw new AppErrorException({ code: "JUDGE_PARSE_FAILED", message: "评分模型未返回有效 JSON" });
  }
...
export async function scoreGeneration(generationId: string, opts: ScoreOptions = {}): Promise<QualityScore> {
  ...
  const selfEvaluated = !completion.model || !generation.model || completion.model === generation.model;
