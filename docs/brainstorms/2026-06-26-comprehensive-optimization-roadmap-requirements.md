...
> **北極星成果指標**：整份路線圖服務同一個用戶成果——**「從生成到一篇可發布稿所需的重生成/改稿次數下降」**。穩定與體驗讓打磨不被打斷、功能讓打磨更省力；P3/P4 是否值得做，以此為準繩。⚠️ 此指標目前**無埋點**，閘門是定性自評（見 Key Decisions「階段重估閘門」）；想要數據可在 P1 加一個輕量計數。
...
> **可獨立發布的增量**：P1 + P2 合起來就是一個對使用者有感、可單獨發布的版本（穩定收尾 + 體驗成品感），不必等到 P3/P4 才交付價值。（此承諾**暫定**——若 S4 穩定複核翻出回歸，P1 範圍與發布時程一併修訂。）每個階段結束都是一個重估點（見 Key Decisions「階段重估閘門」）。
>
...
- S3. `generation.update()` 以事務包裹（修正真實 race）：真正的競爭**不是多用戶**，而是**同一用戶內** cancel 請求（status→cancelled）與串流完成 update（status→completed）對同一 generation 的 read-modify-write 競爭——目前僅靠事務外的 `canTransition` 守衛。修法須把 `get(id)` 的存在性 + `canTransition` 讀取與寫入**一起移進同一個 `db.transaction(...)`**（沿用 `delete()` 已有的 `tx` 同步交易模式；注意 better-sqlite3 交易是同步，現行 `await get()` 的 async 讀寫要轉同步——比「包一層」略多但非阻塞）。（原「單用戶風險低」的理由誤判了風險來源。）
- S4.（P1 首件）**穩定複核**：在把穩定線定為「收尾」前，逐條核對舊 `comprehensive-iteration` 的 **R1 / R3（Playwright E2E 在 `src/` 樹仍可跑且已接 CI）/ R7-R10 / R14-R23** 在現 `src/` 樹的現況，並判定 S1 flaky 的根因是延遲還是 race。複核若翻出仍成立的回歸，補進穩定線（可能重開範圍）。這是便宜的前置保險，先做。**注意**：因 S4 結果可能重開範圍，「P1 成員、P1+P2 可獨立發布、不大改穩定架構」這三項承諾在 S4 跑完前都是**暫定**，S4 若翻出回歸則一併修訂。
- S5. **資料持久性：DB 整檔備份 + 還原（含密鑰）** — 本地單用戶最大的不可逆風險是整個 SQLite 檔損壞/遺失。做成**雙向**流程（只匯出不能還原＝沒護到）：
  - **備份用 raw DB 檔**（better-sqlite3 的 `.backup()` API，WAL 安全；**勿裸 copy `.db`**，最新寫入可能還在 `-wal`）。比 per-table machine-readable 匯出更簡單且天生可還原。
  - **須含加密密鑰目錄** `~/.post-generator/secrets/`——API key **不在 DB**（DB 只存 `apiKeyRef`+遮罩），否則還原後 provider 憑證全壞。**跨機還原需兩端設 `POST_GENERATOR_SECRET_KEY`**（預設密鑰由 username+homedir 派生，換機解不開）；若不含密鑰，成功標準須改為「除 API key 需重新輸入外完整可還原」。
  - **還原是覆寫性操作** → 套 H1 級確認；schema 版本不符 / ID 衝突的處理見 Deferred。
  - **複用既有** `getBackupsDir()`/`getExportsDir()` 與 `export-service.ts`（目錄已建但未用），勿另起爐灶。
  - **過渡零成本**：in-app 還原落地前，README 記錄「整個 `~/.post-generator/` 目錄複製即備份」（Time Machine/git 亦涵蓋）。
...
- H3. 生成狀態與錯誤字串全面 i18n：`use-generation-stream.ts` 有 8 處硬編碼狀態英文（Ready / Generating / Regenerating / Streaming response / Tokens received / Streaming failed / Cancelled / unavailable），**外加錯誤路徑**的 "Streaming response unavailable" / "Streaming failed"（行號隨分支漂移，規劃時 grep 重新定位），以及直接渲染的原始 provider 錯誤訊息（常為英文）。全部納入 `messages/*.json`。**「包裝原始 provider 錯誤訊息」這個子任務一併納入 P1（H3）**，不延到 P2 的 H6——否則「P1 後 zh-CN 全程無英文殘留」會反過來依賴 P2 才成立（P1 依賴 P2 的反模式）。H6 只負責失敗 UI 的呈現/重試，文字 i18n 屬 H3。
...
- F2.（**按需才做、可降級** — 對自用單用戶價值未證實，標為與 F5 同級的選配；想做才做）LLM-as-Judge 質量評分（= 006 Unit 9）：實作既有 `generator-quality-spec.md` 的五維 rubric 評分，含偏差緩解（用不同 model、二元標籤、單輸出評分），以「試讀者建議」口吻呈現而非權威判決；按需觸發、不阻塞生成、結果持久化。**前置環境假設**：偏差緩解的「不同 model」需使用者**已配置 ≥2 個 provider/model**；若只有一個（如僅 Ollama），fallback 為**「停用評分並提示」**（唯一與偏差前提一致的選項——「同模型打折」不採，因自評扭曲的是相對排序而非可減的純量，打折仍是不可信訊號）。
...
- F5.（可選增強，不阻塞）多變體並排對比（U10）：時間/預算允許才納入，否則順延，不阻塞主線。（**U8 大綱優先已完成**，不再屬此選配項。）

## Success Criteria

- **P1 後**：舊 R1-R23（含 R3 E2E）已逐條複核並記錄現況、回歸（若有）已收進範圍（S4）；`pnpm test` + CI 穩定全綠、S1 根因已判定（S1）；**備份→還原可往返**：匯出再還原後 DB 與密鑰皆可用（或明文告知 API key 需重新輸入），覆寫前有確認（S5）；餵給 adapter 一個結構異常但可 parse 的回應會浮出可觀測錯誤而非靜默吞 token（S2）；**cancel 與串流完成在同一交易內仲裁、無 race**（S3）；刪除任一實體都需確認（H1）；zh-CN 下生成全程（含失敗路徑、原始 provider 錯誤訊息）無英文殘留（H3）；F1 控制項已在 prompt 預覽反映且標籤已 i18n（F1 殘留驗證）。
...
- **不重寫** 已過時的 `comprehensive-iteration-requirements.md` — 其多數項目已完成；其 R13 的殘留部分已收窄併入本文件 S2，R1/R3/R7-R10/R14-R23 的現況由 S4 複核（不預設已全解）。
- **資料持久性已納入**：整個 SQLite 檔損壞/遺失才是最大不可逆風險（>誤刪單筆），故新增 S5「DB 備份/全量匯出」進 P1，而非默默略過。
- **可選/可降級**：F2（評分，按需）、F5（U10 多變體；U8 大綱已完成）、H7（行動端整項）— 時間緊可順延，不阻塞主線。

## Key Decisions

- **「完整計劃」= 完整地圖，不等於承諾走完每條路**：三條線都納入是為了給你一張全景圖，但**範圍納入 ≠ 序列承諾**。用 P1→P4 排序確保快贏與高危修復先交付。
- **階段重估閘門（明確為「定性自評」，非數據驅動）**：P1（+P2）交付後是一個明確重估點——先把「穩定收尾 + 體驗成品感」這個可發布增量做出來、確認工具值得繼續深化，再決定是否投入 P3 版本工作流與 P4 選配。**北極星指標目前無埋點可讀**（單機自用、無分析面），故閘門是**開發者本人的定性自評**；若想要證據支撐，可在 P1 順手加一個極輕量的「每篇重生成/編輯次數」計數（持久化於 draft）——否則別把它包裝成「指標驅動」。目的是避免靠慣性走完四階段、過度投資一個自用工具。
...
- 目前分支 `feat/generation-controls-unit7`、HEAD `812ca557`。**U1-U8 已提交完成**（以已提交程式碼/測試核實）。⚠️ 完成清單時效以小時計——本文件兩輪審查間 HEAD 已從 `2f19c20b` 推進到 `812ca557`（多出 U8）；**規劃前務必 `git log` 重核，並把所有 file/line 引用重新解析**（分支推進會讓行號漂移）。
...
- [影響 F2][技術] 評分用哪個 provider/model（單一 model 時已定為「停用+提示」）。
- [影響 S2][Needs research] `parseChunk` 欄位驗證用手寫 type guard 還是 Zod（逐 provider 評估 bundle/維護成本；疊在現有 `safeParseChunk` 上）。
- [影響 S5][技術] 還原時 schema 版本不符 / generation→draft→active_draft_id 的 ID/FK 衝突如何處理；備份是否含 secrets 目錄（含＝完整可還原但跨機需設金鑰；不含＝key 需重輸）。
- [影響 F3/F4][設計] 「working draft」與「version」的概念關係：是一篇共用一個 working draft（切換＝先快照），還是每版各一個（切換即安全）？「auto-save-first」的對象（存成新版本 vs 沖洗暫存）需定義，否則切換/恢復的保護語義不明。
- [影響 F3/H1][設計] 版本的生命週期：可否刪除/重新命名版本（若可刪→屬 H1 破壞性確認範圍，目前 H1 清單未含版本）；版本切換器在「尚無版本」時的空狀態。
