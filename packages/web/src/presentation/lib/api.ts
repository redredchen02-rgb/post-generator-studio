...
import type { Generation, GenerationPreset, PromptTemplate, ProviderProfile } from "@postgen/domain";
...
export async function loadGenerations(search?: string, offset?: number, limit?: number): Promise<{ items: Generation[]; total: number }> { const items = await client.listGenerations(limit ?? 50); return { items, total: items.length }; }
