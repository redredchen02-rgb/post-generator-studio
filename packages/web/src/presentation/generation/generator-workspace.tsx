...
import { stripMarkdown } from "@postgen/sdk";
import { extractTemplateVariables } from "@postgen/sdk";
...
export function GeneratorWorkspace(): React.ReactElement {
  ...
  // Persist custom var values per template via localStorage
  React.useEffect(() => {
    const saved = localStorage.getItem(`pgs-cvar-${templateId}`);
    if (saved) {
      try { setCustomVarValues(JSON.parse(saved)); } catch { setCustomVarValues({}); }
    } else {
      setCustomVarValues({});
    }
  }, [templateId]);
  React.useEffect(() => {
    if (templateId && Object.keys(customVarValues).length > 0) {
      localStorage.setItem(`pgs-cvar-${templateId}`, JSON.stringify(customVarValues));
    }
  }, [customVarValues, templateId]);
